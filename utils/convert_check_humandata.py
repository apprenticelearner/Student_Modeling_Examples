...
# Drop pretest and posttest transactions. And prior knowledge.
if('Level (ProblemSet)' in df):
    df = df[~df['Level (ProblemSet)'].isin(['Pretest', 'Midtest A', 'Midtest B', 'Posttest', 'DPosttest'])]

if('Level (Domain)' in df):
    df = df[~df['Level (Domain)'].isin(['prior_knowledge'])]
...
L = len(df)
for row in range(L):
    #Fix possible issue of missing data in hints
    sel = df.iloc[row]['Selection']
    if(df.iloc[row]['Outcome'] == "HINT" and sel != sel and row < L):
        df.at[row, 'Selection'] = df.iloc[row+1]['Selection']
        df.at[row, 'Action'] = df.iloc[row+1]['Action']
        df.at[row, 'Input'] = df.iloc[row+1]['Input']
        df.at[row, 'Student Response Type'] = "HINT_REQUEST"
...
print(df.iloc[0]['Step Name'],type(df.iloc[0]['Step Name']))
if(isinstance(df.iloc[0]['Step Name'],(int,np.int8,np.int16,np.int32,np.int64))):
    df = df.rename(columns = {'Step Name':'CF (step_id)'})
else:
