...
plot_em <- function(al_curve, human_curve,name='dt'){
  ...
  ca <- plot_from_aggregate(al_curve, mode = "aggregated", y_range = c(0, 0.4), main = "", size = 0.8, legendpos='right')
...
split <- function(agg_results, order, rename, title = "", legend_title = "Dataset", line_size = 0.8, point_size = 1.9, y_range = c(0, 0.4)){
...
hint.plot <- plot.by.error$"Hint" + ylim(0, 0.2) + theme(legend.title = element_blank(),
    ...
    plot.title = element_text(margin = margin(t = 10, b = -20),hjust=1)) +
    ggtitle("Hint-Error")
then.plot <- plot.by.error$"Then Error" + ylim(0, 0.2) + theme(legend.title = element_blank(), plot.title = element_text(margin = margin(t = 10, b = -20),hjust=1)) + 
    ggtitle("Input-Error")
if.plot <- plot.by.error$"If Error" + ylim(0, 0.2) + theme(legend.title = element_blank(), plot.title = element_text(margin = margin(t = 10, b = -20),hjust=1), axis.title.y = element_blank() ) +
...
all_plots <- ggarrange(total.plot, hint.plot, then.plot, if.plot,  nrow=2, ncol=2, common.legend = TRUE, legend="bottom")
...
out <- justHtr$"Total Error"  + theme(legend.title = element_blank()) + scale_x_continuous(limits = c(0, 24)) + labs(title = "Human vs. AI error reduction with repeated practice and instruction")
...
#                       labels=c("Control", "Treatment 1", "Treatment 2"))#+ scale_colour_manual(values = c("#E69F00", "#56B4E900", "#D55E00", "#0072B2","#CC79A7","#1B9E77")) + scale_fill_manual(labels=c("Control", "Treatment 1", "Treatment 2"))
```
